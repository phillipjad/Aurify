...
// Query identifies the generation run whose image is wanted.
//
...
// check against. Revision ids are UUIDv4, which is the same unguessable-name
// trade docs/adr/0014-hosted-generation-apis.md accepted for its public bucket.
type Query struct {
	RevisionID string
}
...
func (h *Handler) Handle(ctx context.Context, q Query) (domain.GeneratedImage, error) {
	return h.images.Find(ctx, q.RevisionID)
}
