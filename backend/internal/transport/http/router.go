...
func NewRouter(deps Deps) (*mux.Router, error) {
	...
	err := router.Configure(func(r *mux.Router) {
		...
		api.POST("/covers", covers.Generate).
			...
			// One run at a time per playlist: a request that arrives while one
			// is already in flight is refused rather than queued.
			WithResponse(409, mux.ProblemDetails{})
		...
		api.GET("/covers", covers.List).
			...
			WithSummary("List the current user's covers, one per playlist, newest first").
			WithQueryParam("status", "Filter to one lifecycle status (empty = all)", "ready").
			WithQueryParam("limit", "Page size (default 20, max 100)", "20").
			WithQueryParam("before", "Keyset cursor: the previous page's last updatedAt", "2026-08-09T12:00:00Z").
			WithQueryParam("before_id", "Keyset cursor: the previous page's last id", "0f8fad5b-d9cb-469f-a165-70867728950e").
			WithOKResponse([]dto.CoverResponse{})

		api.GET("/covers/{id}", covers.Get).
			WithOperationID("getCover").
			WithSummary("Get a cover by id, with its run history").
			WithPathParam("id", "Cover id", "0f8fad5b-d9cb-469f-a165-70867728950e").
			WithQueryParam("allow_failed_revisions", "Include runs that produced no artwork", "true").
			WithOKResponse(dto.CoverResponse{}).
		...
		api.GET("/covers/{id}/image", covers.Image).
			...
			WithSummary("Serve one generation run's image bytes").
			WithPathParam("id", "Revision id, as carried by any imageUrl", "0f8fad5b-d9cb-469f-a165-70867728950e").
			WithResponse(200, nil).
			WithResponse(404, mux.ProblemDetails{})

		api.DELETE("/covers/{id}", covers.Delete).
			WithOperationID("deleteCover").
			WithSummary("Delete a cover and every run it has had").
			WithPathParam("id", "Cover id", "0f8fad5b-d9cb-469f-a165-70867728950e").
			WithNoContentResponse().
			WithResponse(404, mux.ProblemDetails{})
		...
		api.DELETE("/covers/{id}/revisions/{revisionId}", covers.DeleteRevision).
			...
			WithPathParam("id", "Cover id", "0f8fad5b-d9cb-469f-a165-70867728950e").
			WithPathParam("revisionId", "Revision id", "9d3c1e2a-0b44-4b7e-8a71-1c5f0e2d3a84").
			WithNoContentResponse().
			WithResponse(404, mux.ProblemDetails{})
