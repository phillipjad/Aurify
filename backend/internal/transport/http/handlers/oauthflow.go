...
//
// It uses the __Host- prefix, which the browser enforces: the cookie must be
...
const oauthFlowCookie = "__Host-aurify_oauth"

// flowTTL bounds how long a half-finished sign-in stays completable. It only has
// to cover a trip through Google's consent screen.
...
// setFlowState writes the flow cookie.
func setFlowState(c mux.RouteContext, f flowState, secure bool) error {
...
// clearFlowState removes the flow cookie. It is called on both the success and
// failure paths so a spent or abandoned attempt cannot be retried.
func clearFlowState(c mux.RouteContext, secure bool) {
	c.Cookies().Set(flowCookieName(secure), "", -1, "/", "", secure, true, http.SameSiteLaxMode)
}

func readFlowState(c mux.RouteContext, secure bool) (flowState, error) {
	v, err := c.Cookies().Get(flowCookieName(secure))
	if err != nil {
		return flowState{}, errFlowState
	}
...
// flowCookieName drops the __Host- prefix for local HTTP development, where a
// browser would reject a prefixed cookie that is not marked Secure and the flow
